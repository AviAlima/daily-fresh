'use strict';
const { chromium } = require('/Users/avi.alima/.nvm/versions/node/v20.10.0/lib/node_modules/playwright');
...
(async () => {
  ...
  const tasks = await page.$$eval('#taskList .task', els => els.map(e => e.querySelector('.task-text').textContent));
  ...
  const doneCount = await page.$eval('#doneWrap', el => !el.classList.contains('hidden'));
  if (!doneCount) throw new Error('done section should be visible');
  const doneText = await page.$$eval('#doneList .task-text', els => els.map(e => e.textContent));
  console.log('done:', JSON.stringify(doneText));

  // estimate chip (task with estimate is in the done list now)
  const est = await page.$$eval('#taskList .est-chip, #doneList .est-chip', els => els.map(e => e.textContent));
  ...
  if (ver !== 'v21') throw new Error('version mismatch: ' + ver);

  // history view
  await page.click('#navHistory');
  await page.waitForTimeout(200);
  const historyVisible = await page.$eval('#historyList', el => el.children.length >= 1);
  console.log('history card rendered:', historyVisible);

  // reload persistence
  await page.reload({ waitUntil: 'networkidle' });
  await page.waitForTimeout(200);
  const persisted = await page.$$eval('#taskList .task', els => els.length);
  const persistedDone = await page.$$eval('#doneList .task', els => els.length);
