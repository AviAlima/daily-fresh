...
async function openTasks(page) {
  return page.$$eval('#taskList .task', els => els.map(e => e.querySelector('.task-text').textContent));
}
async function doneTasks(page) {
  await page.click('#doneToggle', { force: true }).catch(() => {});
  await sleep(200);
  return page.$$eval('#doneList .task', els => els.map(e => e.querySelector('.task-text').textContent));
}
...
(async () => {
  ...
  await sleep(7000);
  const code = await desk.page.$eval('#syncCode', el => el.textContent.trim());
  ...
  await sleep(3000);
  const phPaired = await ph.page.$eval('#syncBody', el => !el.classList.contains('hidden'));
  ...
  await sleep(4000);
  let phTasks = await openTasks(ph.page);
