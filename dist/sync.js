...
(function () {
    ...
    const FIELDS = ['done', 'text', 'carriedFrom', 'estimate', 'order'];
    let app = null;
    let auth = null;
    let db = null;
    let user = null;
    ...
    function getSync() {
        if (!Sync.state) {
            try {
                Sync.state = JSON.parse(localStorage.getItem(SYNC_KEY) || 'null');
            }
            catch (e) {
                Sync.state = null;
            }
    ...
    function readSyncLog() {
        if (!syncLog) {
            try {
                syncLog = JSON.parse(localStorage.getItem(LOG_KEY) || 'null');
            }
            catch (e) {
                syncLog = null;
            }
    ...
    function deepEq(a, b) {
        ...
        if (a && b && typeof a === 'object' && typeof b === 'object') {
            ...
            for (let i = 0; i < ka.length; i++) {
                if (!(ka[i] in b) || !deepEq(a[ka[i]], b[ka[i]]))
                    return false;
            }
            return true;
        }
    ...
    function normDay(d) {
        ...
        if (!d.orderTs)
            d.orderTs = 0;
        (d.tasks || []).forEach((t) => { if (!t.ts)
            t.ts = {}; });
        return d;
    }
    function maxTs(ts) {
        let m = 0;
        Object.keys(ts || {}).forEach((k) => { if (ts[k] > m)
            m = ts[k]; });
        return m;
    }
    ...
    function mergeTombstones(a, b) {
        ...
        (b || []).forEach((t) => {
            if (t.id in byId)
                byId[t.id] = Math.max(byId[t.id], t.deletedAt);
            else
                byId[t.id] = t.deletedAt;
        });
        const out = [];
    ...
    function mergeTask(lt, rt) {
        ...
        if ((rts.done || 0) > (lts.done || 0))
            out.doneAt = rt.doneAt || null;
        if (!out.ts)
            out.ts = {};
        Object.keys(rts).forEach((k) => {
            if ((rts[k] || 0) > (out.ts[k] || 0))
                out.ts[k] = rts[k];
        });
    ...
    function mergeDay(local, remote) {
        ...
        ['note', 'focus', 'reflection'].forEach((f) => {
        ...
        (remote.tasks || []).forEach((rt) => {
            if (localById[rt.id])
                return;
            if (isTombstoned(day.tombstones, rt.id, rt.ts))
                return;
        ...
        const pruned = [];
        out.forEach((t) => {
            if (isTombstoned(day.tombstones, t.id, t.ts)) {
                changed = true;
                return;
            }
            pruned.push(t);
    ...
    function applyRemote() {
        ...
        if (!state.days)
            state.days = {};
        if (!state.settings)
            state.settings = { resetHour: 0, theme: 'dark', sound: true, name: '' };
        let changed = false;
        Object.keys(remoteDays).forEach((k) => {
            let local = state.days[k];
            if (!local)
                local = state.days[k] = newDay();
    ...
    function pushDay(day, remote, now) {
        ...
        ['note', 'focus', 'reflection'].forEach((f) => {
            if (remote && deepEq(day[f] || '', remote[f] || '')) {
                if (!doc.fieldTs[f] && remote.fieldTs)
                    doc.fieldTs[f] = remote.fieldTs[f] || now;
            }
            else {
                doc.fieldTs[f] = now;
            }
    ...
    function prime() {
        return metaRef.get().then((snap) => {
            remoteMeta = snap.exists ? snap.data() : null;
            return daysCol.get().then((qs) => {
                remoteDays = {};
                qs.forEach((d) => { remoteDays[d.id] = d.data(); });
                initialized = true;
                touch();
    ...
    function startListeners() {
        stopListeners();
        unsubs.push(daysCol.onSnapshot((snap) => {
            remoteDays = {};
            snap.forEach((d) => { remoteDays[d.id] = d.data(); });
        ...
        unsubs.push(metaRef.onSnapshot((snap) => {
            remoteMeta = snap.exists ? snap.data() : null;
            pendingSyncWrites = pendingSyncWrites || !!(snap.metadata && snap.metadata.hasPendingWrites);
            if (!(snap.metadata && snap.metadata.fromCache))
                touch();
    ...
    function ensureAuth() {
        ...
        return auth.signInAnonymously().then((r) => { user = r.user; return user; });
    }
    function pair(code) {
        ...
        let st = getSync();
        if (!st || st.code !== codeStr)
            setSyncState({ code: codeStr, hash: hashCode(codeStr), paired: false });
        st = getSync();
        if (!st)
            return Promise.reject(new Error('no-sync-state'));
    ...
    root.Sync = {
        ...
        getStatus: () => {
            const st = getSync();
            if (!st || !st.paired || !db)
                return { state: 'off', lastContact, dirty, error: null };
            if (syncError)
                return { state: 'error', lastContact, dirty, error: syncError };
            if (Sync.online === false || !lastContact || Date.now() - lastContact > STALE_MS) {
                return { state: 'stale', lastContact, dirty, error: null };
            }
            if (dirty || pendingSyncWrites)
                return { state: 'pending', lastContact, dirty, error: null };
