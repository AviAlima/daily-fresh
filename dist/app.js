...
(function () {
    ...
    function migrate(p) {
        ...
        if (p.settings) {
            if (typeof p.settings.resetHour === 'number')
                s.settings.resetHour = p.settings.resetHour;
            if (typeof p.settings.theme === 'string')
                s.settings.theme = p.settings.theme;
            if (typeof p.settings.sound === 'boolean')
                s.settings.sound = p.settings.sound;
            if (p.settings.name)
                s.settings.name = p.settings.name;
        }
        if (Array.isArray(p.tomorrow))
            s.tomorrow = p.tomorrow;
        ...
        if (typeof p.tomorrowTs === 'number')
            s.tomorrowTs = p.tomorrowTs;
        if (typeof p.nameTs === 'number')
            s.nameTs = p.nameTs;
        if (typeof p.resetHourTs === 'number')
            s.resetHourTs = p.resetHourTs;
        return s;
    }
    ...
    function save() {
        ...
        if (window.Sync && window.Sync.onLocalChange)
            window.Sync.onLocalChange();
    }
    ...
    function parseEstimate(text) {
        ...
        if (m)
            return Math.round(parseFloat(m[1]) * 60 + parseInt(m[2] || '0', 10));
    ...
    function toggleTask(id) {
        ...
        if (task.done) {
            ...
            completeChime();
            if (allDone())
                setTimeout(function () { toast('All done \u2014 enjoy your day'); }, 300);
            else
                toast('Completed', { label: 'Undo', fn: doUndo });
        }
        else {
            task.doneAt = null;
        }
    ...
    function carryCandidates() {
        ...
        todayTasks.forEach(function (t) {
            if (t.carriedFrom)
                carried[t.carriedFrom.day + ':' + t.carriedFrom.id] = true;
        });
    ...
    function carryTask(dayKey, task, silent) {
        ...
        render();
        if (!silent)
            toast('Added to today', { label: 'Undo', fn: doUndo });
    }
    function carryAll() {
    ...
    function doUndo() {
        ...
        undoSnapshot = null;
        if (!state.days[snap.dayKey])
            state.days[snap.dayKey] = newDayObj();
    ...
    function applyTheme() {
        ...
        if (meta)
            meta.setAttribute('content', resolved === 'dark' ? '#151a21' : '#f4f5f8');
    }
    ...
    function renderSyncStatus() {
        ...
        if (!pill || !window.Sync || !window.Sync.getStatus || !window.Sync.isConfigured())
            return;
        ...
        if (els.syncPillLabel.textContent !== text)
            els.syncPillLabel.textContent = text;
    }
    function renderSync() {
        if (!els.syncRow || !window.Sync)
            return;
    ...
    function renderCarry() {
        ...
        els.carryList.innerHTML = candidates.map(function (c) {
            return ('<button class="carry-item" data-carry="' + c.day + '" data-carry-id="' + c.task.id + '">' +
    ...
    function renderTomorrow() {
        ...
        els.tomorrowList.innerHTML = list.map(function (t) {
            return ('<li class="tomorrow-item" data-id="' + t.id + '">' +
    ...
    function lastKeys(n) {
        var keys = [];
        for (var i = 0; i < n; i++)
            keys.push(shiftKey(activeDay, -i));
        return keys.reverse();
    }
    function dayStats(key) {
        var d = state.days[key];
        if (!d || !d.tasks.length)
            return { total: 0, done: 0, ratio: 0, exists: false };
    ...
    function renderHistory() {
        ...
        els.weekChart.innerHTML = last7.map(function (key) {
            ...
            return ('<div class="chart-col" title="' + fullDateLabel(key) + (st.total ? ': ' + st.done + ' of ' + st.total + ' done' : ': no tasks') + '">' +
    ...
    function renderRecap() {
        ...
        if (best.n > 0)
            line += ' \u00b7 best day: <strong>' + dayLabel(best.key) + '</strong> (' + best.n + ')';
        line += '.';
    ...
    function renderCalendar() {
        ...
        for (var i = 0; i < startDow; i++)
            html += '<span class="cal-day empty-cell">.</span>';
    ...
    function toast(msg, action) {
        ...
        clearTimeout(toastTimer !== null && toastTimer !== void 0 ? toastTimer : undefined);
    ...
    function onTaskClick(e) {
        ...
        if (check) {
            toggleTask(check.dataset.check || '');
            return;
        }
        var del = t.closest('[data-del]');
        if (del) {
            removeTask(del.dataset.del || '');
            return;
        }
    ...
    els.noteInput.addEventListener('input', function () {
        ...
        clearTimeout(noteSaveTimer !== null && noteSaveTimer !== void 0 ? noteSaveTimer : undefined);
    ...
    function addTomorrow() {
        ...
        text.split(/\r?\n/).forEach(function (line) {
            if (line.trim())
                state.tomorrow.push({ id: uid(), text: line.trim() });
        });
    ...
    els.tomorrowList.addEventListener('click', function (e) {
        ...
        if (!t)
            return;
        var doneBtn = t.closest('[data-t-done]');
        if (doneBtn) {
        ...
        var delBtn = t.closest('[data-t-del]');
        if (delBtn) {
    ...
    els.historyList.addEventListener('click', function (e) {
        ...
        if (!t)
            return;
        var btn = t.closest('[data-del-day]');
        if (!btn)
            return;
    ...
    els.calGrid.addEventListener('click', function (e) {
        ...
        if (!t)
            return;
        var cell = t.closest('[data-cal-day]');
    ...
    els.syncUnpair.addEventListener('click', function () {
        if (!confirm('Stop syncing? Tasks on this device stay here.'))
            return;
        window.Sync.unpair();
        render();
        toast('Sync stopped');
    ...
    $('resetDataBtn').addEventListener('click', function () {
        if (!confirm('Erase all tasks, notes and history? This cannot be undone.'))
            return;
        state = {
    ...
    els.importFile.addEventListener('change', function () {
        ...
        reader.onload = function () {
            try {
                ...
                if (data && data.state && typeof data.state.days === 'object')
                    data = data.state;
    ...
    function beginDrag(li, isTouch) {
        clearTimeout(touchTimer !== null && touchTimer !== void 0 ? touchTimer : undefined);
        pressOrigin = null;
        dragIsTouch = isTouch;
        suppressClick = true;
    ...
    els.taskList.addEventListener('pointerdown', function (e) {
        ...
        if (!t)
            return;
        var li = t.closest('.task');
        if (!li || li.classList.contains('done'))
            return;
        if (t.closest('.drag-handle'))
            beginDrag(li, false);
    });
    window.addEventListener('pointermove', function (e) {
        if (dragState && !dragIsTouch)
            applyDrag(e.clientX, e.clientY);
    });
    window.addEventListener('pointerup', function () { if (!dragIsTouch)
        endDrag(true); });
    window.addEventListener('pointercancel', function () { if (!dragIsTouch)
        endDrag(false); });
    ...
    els.taskList.addEventListener('touchstart', function (e) {
        ...
        if (!t)
            return;
        var li = t.closest('.task:not(.done)');
        if (!li)
            return;
        suppressClick = false;
        var tt = e.touches[0];
    ...
    function applyDrag(clientX, clientY) {
        ...
        if (!dragState.moved && Math.abs(dyTotal) < 7)
            return;
    ...
    function edgeScroll(clientY) {
        ...
        var speed = 0;
        if (clientY < zone)
            speed = (zone - clientY) * 0.35;
        else if (clientY > vh - zone)
            speed = -(clientY - (vh - zone)) * 0.35;
        clearInterval(autoScrollTimer !== null && autoScrollTimer !== void 0 ? autoScrollTimer : undefined);
    ...
    function updateTimerUI() {
        ...
        var timerEl = modal.querySelector('.timer');
    ...
    function openFocusModal() {
        ...
        var modalEl = $('focusModal').querySelector('.modal');
    ...
    function syncTimer() {
        ...
        if (timer.remaining <= 0) {
            ...
            timer.running = false;
            timer.endAt = null;
            clearInterval((_a = timer.interval) !== null && _a !== void 0 ? _a : undefined);
            focusChime();
    ...
    function resetIdleTimer() {
        clearTimeout(idleTimer !== null && idleTimer !== void 0 ? idleTimer : undefined);
        idleTimer = setTimeout(function () {
            if (!isTouchScreen() || currentView !== 'today')
                return;
            if (document.body.classList.contains('keyboard-open'))
                return;
            idleCollapsed = true;
    ...
    if (window.visualViewport) {
        ...
        window.visualViewport.addEventListener('resize', function () {
            clearTimeout(kbdSettleTimer !== null && kbdSettleTimer !== void 0 ? kbdSettleTimer : undefined);
